 ...
 * 
 * <p>Assuming that the shared memory (any objects, primitives, etc) are only accessed through 
 * the same instance of {@link TaskExecutorDistributor}, and assuming that those variables are 
 * only accessed via the same key.  Then the programmer does not need to worry about 
 * synchronization, or volatile.  The {@link TaskExecutorDistributor} will ensure the 
